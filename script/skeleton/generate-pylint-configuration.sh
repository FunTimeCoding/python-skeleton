...
pylint --reports y --disable missing-docstring,redefined-variable-type --max-returns 1 --max-args 3 --max-module-lines 300 --max-statements 10 --max-line-length 80 --min-public-methods 1 --max-public-methods 10 --max-parents 3 --max-attributes 3 --max-locals 6 --max-branches 5 --max-bool-expr 1 --max-nested-blocks 3 --ignored-argument-names "${NON_MATCHING_EXPRESSION}" --ignore-long-lines "${NON_MATCHING_EXPRESSION}" --good-names '' --bad-names "${BAD_NAMES}" --notes TODO --expected-line-ending-format LF --no-space-check '' --ignore .git,.venv --persistent no --spelling-dict "${DICTIONARY}" --spelling-private-dict-file tmp/combined.dic --generate-rcfile >.pylintrc
